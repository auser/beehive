...
proxy_init(ClientSock) ->
  receive
    {start, ClientSock, RequestPid} ->
      ...
      GetBee = ?BENCHMARK_LOG("Getting bee for routing key", beehive_router_srv, get_bee, [RoutingKey]),
...
engage_bee(ClientSock, RequestPid, RoutingKey, ForwardReq, Req, {ok, #bee{host = Host, port = Port} = Bee}) ->
  ...
  case gen_tcp:connect(Host, Port, SockOpts, ?CONNECT_TIMEOUT) of
    ...
    {error,econnreset} ->
      timer:sleep(200),
      GetBee = ?BENCHMARK_LOG("Getting bee for routing key after a connection reset", beehive_router_srv, get_bee, [RoutingKey]),
      engage_bee(ClientSock, RequestPid, RoutingKey, ForwardReq, Req, GetBee);
    Error ->
      ?LOG(error, "Connection to remote TCP server: ~p:~p ~p", [Host, Port, Error]),
      ?NOTIFY({bee, cannot_connect, Bee#bee.id}),
      timer:sleep(1000),
      GetBee = ?BENCHMARK_LOG("Getting bee for routing key after an error", beehive_router_srv, get_bee, [RoutingKey]),
      engage_bee(ClientSock, RequestPid, RoutingKey, ForwardReq, Req, GetBee)
  end;
