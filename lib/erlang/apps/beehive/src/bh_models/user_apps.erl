...
from_proplists([{user_email, V}|Rest], UserApp) -> from_proplists(Rest, UserApp#user_app{user_email = V});
from_proplists([{app_name, V}|Rest], UserApp) -> from_proplists(Rest, UserApp#user_app{app_name = V});
from_proplists([{level, V}|Rest], UserApp) -> from_proplists(Rest, UserApp#user_app{level = V});
from_proplists([_Else|Rest], UserApp) -> from_proplists(Rest, UserApp).

to_proplist(UserApp) -> to_proplist(record_info(fields, user_app), UserApp, []).
to_proplist([], _UserApp, Acc) -> Acc;
to_proplist([user_email|Rest], #user_app{user_email = Id} = UserApp, Acc) -> to_proplist(Rest, UserApp, [{user_email, Id}|Acc]);
to_proplist([app_name|Rest], #user_app{app_name = Name} = UserApp, Acc) -> to_proplist(Rest, UserApp, [{app_name, Name}|Acc]);
to_proplist([level|Rest], #user_app{level=Level} = UserApp, Acc) -> to_proplist(Rest, UserApp, [{level, Level}|Acc]);
...
% Validate the level
validate_user_app([level|Rest], #user_app{level = undefined} = UserApp) -> validate_user_app(Rest, UserApp#user_app{level = ?REGULAR_USER_LEVEL});
