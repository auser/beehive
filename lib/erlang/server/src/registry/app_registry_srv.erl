...
list_applications() -> gen_server:call(?SERVER, {list_applications}).

get_unused_port() -> gen_server:call(?SERVER, {get_unused_port}).
...
handle_call({find_application, AppName}, _From, State) ->
	...
	{reply, Reply, State};
handle_call({get_unused_port}, _From, State) ->
  Reply = handle_get_unused_port(State),
  {reply, Reply, State};
handle_call({list_applications}, _From, State) ->
  Reply = handle_list_applications(State),
  {reply, Reply, State};
...
handle_list_applications(Table) ->
  ?TRACE("tab2list", [ets:tab2list(Table)]),
  ets:tab2list(Table).

handle_get_unused_port(Table) ->
  ?TRACE("tab2list", [ets:tab2list(Table)]),
  5001.
