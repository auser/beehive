...
module Beehive
  module Command
    ...
    class SetupBase < Base
      ...
      attr_reader :options, :type_of_repos
      
      def run
        parse_args do |opts|
          opts.on("--no-nginx") {|n| @no_nginx = true }
          opts.on('-t type', "--type type of repos") {|n| @type_of_repos = n }
        end
      ...
      def rsync_and_setup_remote_directories
        ...
        ssh "#{prefix}/bin/setup-host.sh #{user} #{@type_of_repos}"
